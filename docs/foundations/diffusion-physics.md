...
where $\langle r^2 \rangle$ is the mean squared displacement and $t$ is the observation time. For free water at body temperature, $D \approx 3 \times 10^{-3}$ mm$^2$/s. During a typical diffusion MRI measurement (~50 ms), water molecules travel roughly 10–20 micrometers — about the width of a cell.
...
1. **First gradient pulse**: Applied before the diffusion time. This imparts a position-dependent phase shift to every proton.
2. **Diffusion time**: Protons are allowed to diffuse freely for ~50 ms.
3. **Second gradient pulse**: Applied after the diffusion time. This reverses the phase shift — but only for protons that have not moved.
...
The **Stejskal-Tanner equation** describes this signal attenuation:
...
- $b$ is the **b-value**, which controls how sensitive the measurement is to diffusion (determined by gradient strength, duration, and timing)
...
The eigenvectors define the orientation of an ellipsoid that visualizes the diffusion profile. In coherent white matter, this ellipsoid is elongated like a cigar (prolate). In CSF, it is a sphere (isotropic). In regions with crossing fibers, it may be flattened like a pancake (oblate).
...
However, approximately **60–90% of white matter voxels** contain multiple fiber populations — crossing, kissing, or fanning fibers. In these voxels, the single tensor averages the different fiber orientations into a misleading intermediate direction. This is why:
...
- Le Bihan D, Breton E (1985). Imagerie de diffusion in vivo par resonance magnetique nucleaire. *Comptes Rendus de l'Academie des Sciences*, 301(15), 1109-1112.
