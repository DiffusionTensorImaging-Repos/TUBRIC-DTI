...
## Method 3: HPC Job Scheduler (SLURM)

For shared computing clusters, submit each subject as a separate job:

```bash
#!/bin/bash
# submit_eddy.sh — Submit eddy jobs to SLURM

base_dir="/path/to/project"

for subj_dir in "$base_dir"/denoised/sub-*; do
    subj=$(basename "$subj_dir")

    sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=eddy_${subj}
#SBATCH --output=logs/eddy_${subj}_%j.out
#SBATCH --error=logs/eddy_${subj}_%j.err
#SBATCH --mem=8G
...
module load cuda/11.4  # if using eddy_cuda

./process_eddy.sh "$subj"
EOF
